...
### Primary Design Documents (read these first) The order reflects what was created later. If there is a conflict newer document is right.:
...
3. **`scripts/buildSkills.cjs`** - **NEW!** CSV→JSON build system (heavily commented)
...
4. Attacker triggers feint → morphs to different attack (e.g., side_slash on `horizontal`)
...
### 9. **Skill Renaming: slash → side_slash**
...
else if (elapsed < impactTick) → committed (or active for defense)
...
**Medium Priority:** 6. NPC AI decision tree 7. Combo system (skill chaining) 8. More attack/defense skills 9. Counter attack system (use counterSpeedBonus) 10. Damage system beyond instant death
...
3. Read `scripts/buildSkills.cjs` - See how CSV→JSON works (heavily commented)
