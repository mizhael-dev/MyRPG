...
2. Let's continue disussing and iterate:
   ...
   - 2.5. **Hit Confirmation** needs to be clarified
...
Describe/define below conscepts in the simliar style as I did below for other concepts:
   ...
   - **Press:**
   - **Disengage**
   - actions available when in Strong 
...
3. First `Pause` like `Indes`: P1 and P2 select their Actions blindly (they don't see the opponent's choice) and secretely (the opponent doesn't see it). Available actions:
   ...
   2. `GuardChange`
      ...
      2. `GuardPosition` move to another GuardPosition e.g. postion=[1,1] for high-lef, or [2,2] fo vertical and horizontal center
   ...
   5. `Impact`
      ...
      2. Or `Nach` fencer spends Focus `FP` to catch up (Vor has +1 `LeaverageMeter`)
      3. Or `Nach` fencer allows `Vor` (Vor has +1 `LeaverageMeter`) to perform their action uniterrupted. 
   6. `HitResolution`
      1. Distance Check
      ...
      2. `Indes` `Pause` for both P1 and P2
         ...
         2. `BlindAuction` fencers assing AP to boost either strength or speed
...
Gives a chance to either fencer in `Initiative`=`Vor` or `Initiative`=`Nach` to adjust their `Actions` in light of new information `Telegraphs`.
  ...
  ***Game pauses when:***
  1. `NewTelegraph` New meaningful telegraph information emerges (Every time you see their weapon, foot, or body move)
    ...
    - **Opponent** of the fencer that signals the telegraph can act
...
2. **Nach (After)** -  is the subsequent defensive reaction or counter-tempo action taken by the defender, often resulting in a binding of the blades (an Interrrupt).
    ...
    ***GameEngine:***
    - Fencer who's planned `Impact` is at least 1 `Tick` later on the `Timeline` than the other person's `Impact`, has `Initiative`=`Nach`
