...
1. Combat starts
   1. `AP` resource is set to `MaxAP` for the actor
   2. `FP` resource is set to `MaxFP` for the actor
   3. Actors are `StartDistance` apart
   4. `CurrentTick` time is set to 0
   5. The `CurrentTick` countup is paused
2. First `Pause` like `Indes`: P1 and P2 select their Actions blindly (they don't see the opponent's choice) and secretely (the opponent doesn't see it).
   **Available actions:**
   1. Move forward/backwards
      1. How far actors can move is defined by their `MoveDistance`
      2. The system calculates the `AP cost`
      3. The system calculates `IT` duration (how long it will take to move the desired `Distance`)
   2. `GuardChange`
   ...
   3. `WeaponAction` both players can start their weapon movement
   ...
   4. `Telegraphs` and adjustments by signal receivers
   5. `Impact`
...
Distance is the only factor that can make a hit really miss, whiff.
Moving costs `AP` (effort) and costs `IT` (time) and is a vulnarability (opportunity for opponent to act) `Pause`

**The distance** is calulated from one Actor to the other.
...
***GameEngine:***
  - `Distance` between actors is caluclated on the attack resolution after the movement:
    - `Distance` between 100cm to 140cm - attack misses/whiffs, (out of measure or void)
  ...
  - `StartDistance` combat starts with Actors 280cm apart
  - `Movement`:
    - `MoveDistance`: 46-90cm,`MoveCost`=2, `Pause` = 1
    - `MoveDistance`: 0-45cm, `MoveCost`=1, `Pause` = 1
    - `MoveDistance`: 0-10cm, `MoveCost`=0, `Pause` = 0
  - Each Actor has a `MoveSpeed`, a maximal distance they can move (`MoveIT`) in one `IT`.
  ...
  - Actors move (use legs) and use arms (move sword) at the same time. Action time and move time moves in parralell:
   ...
   - move telegraph will be signaled at the end of first `IT` in which move happened.
  -Actors always move at top speed (their life is on the line)
  ...
  - Actors move is resolved at the same time as `CurrentIT` counts forward
...
#### Distance - Design and reality context
  ...
  2. **Wide Distance (Larga Misura, Zufechten):** Considered Out of Measure for most techniques, where a Actor might be over-reaching, makeing them vulnerable to counter-strikes. A hit is possible, but it requires substantial footwork, typically a committed step-lunge or a full passing step. 
