**Date:** 2026-01-02
**About file:** This is a main file that links to other files that descibe CombatSystemV2

# Combat System V2

Combat System V2 is a combat engine that will be implemented in a realistic sword martial arts computer rpg game.

### Key features of Combat System V2

- **Smart Pauses:** Game is not reflex based, nor truly turn based. The concept is based on Pause "freeze frame", like in the movie 300.
  - **Simultaneous Action Evolution** - Both actors' actions progress in parallel
  - **Telegraphing/Intent Reading** - system provides clues about what the opponent is doing, the closer to execution the more clues are known but there is less time to react which means more resources need to be spent to catch up.
  - **Pause, "freeze frame"** - system pauses time under specified condition and actor can decide what to do. There is no time limit on how long player can think. 
...
- **Fast and deadly** - No HP; combat is brutal and tactical. Wounds accumulate and provide debuffs until one of the actors is unable to continue or dies.

---

### Character Attributes, Derived Stats and Resources:

*see 20260101_Character_stats.md*
...
**INITIAL STATE**
1. Combat State Initialization
   1. P1 and P2 `AP` resource is set to `MaxAP`
   2. P1 and P2 `FP` resource is set to `MaxFP`
   3. Distance = Measure = 1 (Striking distance)
   4. `CurrentTick` time is set to 0
   5. The `CurrentTick` countup is paused
   6. P1 and P2 `GuardPosition` is set to [2,2]

**FIRST PHASE**
*P1 and P2 select their Actions blindly (they don't see the opponent's choice) and secretly (the opponent doesn't see it)*
...
3. Timeflow transistions to:
   1. `CurrentTick` advances at 1x rate
   2. Grid is updated with weapon moving guard (blue vector) 

**P1 INITIATES THREAT**
*P1 and P2 select their Actions blindly (they don't see the opponent's choice) and secretly (the opponent doesn't see it)*

4. Pause for Both
   1. P1 declares `WeaponAction`
      1. `Intent` = Hitter
      2. `AttackPath` = [1,2] -> [3,2]
   2. P1 commits:
      1. `APCommitted_PreIndes` = 5
      2. IT = `Base Time` - (`APCommitted_PreIndes` * `ActionSpeed`)
   3. P2 does nothing

5. Timeflow transistions to:
   1. `CurrentTick` advances at 1x rate
   2. Grid is updated with weapon moving guard (red vector)

**P2 DETECTS THREAT**

6. Pause for P2, after `DetectTime` = 0.25s
   *P2 observes: P1 weapon trajectory. P1 AP investment magnitude (approximate)*
   *P2 infers: Action is likely Hitter or Provoker. P1 speed is moderate. P2 knows their speed stat is too low to compete in speed contest and ecides to change guard and Take*
   1. P2 `GuardPosition`
      1. [1,2] -> [3,2]
      2. Cost:
         1. `AP_spent`: 1
   ...
   2. P2 `WeaponAction`
      1. `Intent` = Taker
      2. `AttackPath` = [3,2] -> [1,2]
   3. P2 commits:
      1. `APCommitted_PreIndes` = 5
      2. IT = `Base Time` - (`APCommitted_PreIndes` * `ActionSpeed`)
   4. System informs P2 that they are 1s late and it will cost X `FP` to catch up
      1. P2 pays 1 `FP_spent`
...
2. Initial `Pause` like `Indes`: P1 and P2 select their Actions blindly (they don't see the opponent's choice) and secretely (the opponent doesn't see it).
   **Available actions:**
   1. `GuardChange`
      1. Change `Guard` to another e.g. from `Guard`=Plow to `Guard`=Wrath 
      2. `GuardPosition` move to another GuardPosition e.g. postion=[1,1] for high-left, or [2,2] for vertical and horizontal center
         1. and takes 0.5 `IT` per 1 square moved and cost 1 `AP`
   2. `WeaponAction` both players can start their weapon movement
      1. `AttackType` and `AttackDirection` weapon ending position on 3x3 grid representing body:
         1. For `Cut` Has to move at least 2 squares from current `GuardPosition`
            1. e.g. from 3x3 grid [1,1] you move (cut) to either [1,3], [2,3] [3,3], [3,1], [3,2],[2,2], you cannot cut to [1,1], [1,2], [2,1],  
            2. or from [2,2] you cut to either [1,1], [1,3], [3,1], [3,3]
         2. For `Stab` max movement one column and one row, or can stay in current `GuardPosition`
            1. e.g. from [1,1] to [1,1] [1,2], [2,1], [2,2]
            2. or from [1,2] to [1,1] [1,2], [1,3], [2,1], [2,2], [2,3]
   3. `Telegraphs` and adjustments by signal receivers
   4. `Impact`
      1. Natural if both players attack in the same tick
      2. Or `Nach` Actor spends Focus `FP` to catch up (Vor has +1 `LeaverageMeter`)
      3. Or `Nach` Actor allows `Vor` (Vor has +1 `LeaverageMeter`) to perform their action uniterrupted. 
   5. `HitResolution`
      1. Distance Check
         1. if not in range miss
         2. If in range hit
      2. `Indes` `Pause` for both P1 and P2
         1. `Intent` Hitter, Taker, Provoker
         2. `BlindAuction` Actors assing AP to boost either strength or speed
      3. If range hit, then Weapon `Collision Check` and `H>P>T>H matrix` comparison
         1. 2 or more zones in collision
               1. Same Intent: 2x**Hitter** or 2x**Taker**
                  1. Opposite direction = strength resolution + AP spent on auction
                     1. `Clash` (range > 100cm) or `Bind` (range =< 100cm)
                        1. `Fuhlen` reveals `LeaverageMeter`
                           1. `MoV` Margin of Victory calculation `MoV`= `Test_Stat` + `APSpentAuction`
                  2. Same direction = speed resolution + AP spent on auction
                     1. `MoV` Margin of Victory calculation `MoV`= `Test_Stat` + `APSpentAuction`
                     2. Final `Mov` table resolution
               2. Same Intent: 2x**Provoker**
                  1. nothing
               3. Rock-Paper-Scissors won
                  1. Winner gets +1 on `LeaverageMeter`
                  2. Loser gets -1 on `LeaverageMeter`
                  
         2. 1 zone in collision
            1. Stark+1 for player with longer movement
         3. 0 zones in collision = speed resolution

!!!!!!!!!
   7. `Focus` If you are in `Nach` -> `Indes` how many Focus Points `FP` are you going to spend to catch up to Opponent that has started the attack first (`Vor`). 1`FP` = 1`IT`. With `FP` of 3 you can be 3 Ticks behind the attacker on the timeline and still make it for the `Impact`
   8. `Impact` hit resolution.
      1. `Indes` (Pause for P1 and P2)
      2. `Fuhlen` (Leaverage revealed)
         1. `AP` EDIT
!!!!!!!!!!
...
In this system, Actors share a common `Timeline` govern by `CurrentTick`. The turn order is fluid and based on `SpentIT` of a total action and the `RecoveryIT` time.
...
- `WeaponAction`: {`Intent`, `AttackDirection`, `SpentAP`,}
- `Focus`: {`SpentFocus`}

Simplified Example (numbers do not match with actual IT cost of actions):
1. Start:Indes (`Pause` for P1 and P2):
...
Gives a chance to either Actor in `Initiative`=`Vor` or `Initiative`=`Nach` to adjust their `Actions` in light of new information `Telegraphs`.
  ...
  ***Game pauses when:***
  1. `NewTelegraph` New meaningful telegraph information emerges (Every time you see their weapon, foot, or body move)
    ...
    - **Opponent** of the Actor that signals the telegraph can act
    - Example: P1 telegraphs → P2 can respond 
  2. `RecoveryComplete` Recovery phase is completed and actor can act again
  ...
  3. `Death` On hp <= 0
    ...
    - Message: "{name} died"
  4. `Indes` Indes starts and the attack is about to be resolved
...
1. **Vor (Before)** - represents the initial attack or aggressive action taken by one Actor, seizing the initiative. This action forces a response from the opponent.
   
   ***GameEngine:***
   - Actor who's planned `Impact` (moment when attack is going to be resolved if no Actions are made) is sooner on the `Timeline` than the other person's `Impact`, has `Initiative`=`Vor`
   - `Initiative`=`Vor` gives a bonus to `Leaverage` +1 Stark
   - If both Actors have expected `Impact` in the same `Tick` on the `Timeline`, neither will get a `Vor` Leavearge bonus when `Indes` is resolved. ***
...
2. **Nach (After)** -  is the subsequent defensive reaction or counter-tempo action taken by the defender, often resulting in a binding of the blades (an Interrrupt).
    ...
    ***GameEngine:***
    - Actor who's planned `Impact` is at least 1 `Tick` later on the `Timeline` than the other person's `Impact`, has `Initiative`=`Nach`
...
3. **Indes (Instant/Meanwhile/instance of stilleness)** - the single most critical element for simulating KdF in an atomic-turn system. Indes translates to "in the instant" or "meanwhile" and represents the ***immediate, reflexive action taken upon contact with the opponent's sword**. This is the precise moment where a Actor, using trained instinct (**Fühlen**) to judge opponent's **Leaverage** (Strong or Weak) and makes a definitive counter-action (a **winding**, a **disengage**, or a **press**).

!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ***GameEngine:*** EDIT EDIT EDIT EDIT !!!!!!!!
  - `Indes` is a `Pause` for both P1 and P2
  -`Fuhlen` the `LeverageTrack` (Stark/Swach position by ) is revealed.
  - The Fühlen Check: When blades contact, the game enters Indes State (Pause). 
  - Before Indes resolution is revealed, Actors have one last chance to edit their Actions and go to Vor Nach again. It will cost 3 `FP` Focus Points to get at least 1 `Tick` on `Timeline` ahead. EDIT EDIT
 ...
 - Winner of Hitter-Taker-Provoker (Rock-Papaer-Scissors), makes the final decision.

---

### Bind vs. blade contact
All Binds are blade contact, but not all blade contact is a Bind.
The distinction comes down to Duration and Stickiness.

### 1. Instant Contact: Deflection / Beat / Parry

This is when blades hit and immediately separate.
* **The Physics:** Like two billiard balls colliding. There is an impact, a "clack" sound, and then the weapons bounce apart.
* **The Tactical Result:** Reset. The attacker failed, distance is usually created, and both players are free to move or swing again.
* **Game Logic:**
    * **Event:** `Attack` meets `Defense`.
    * **Result:** Attack damage = 0.
    * **Next State:** Both players return to "Neutral." The sword is no longer at `[2,2]`; it has recoiled back to a starting position like `[1,3]`.

### 2. The Bind (*Anbinden*): Sustained Contact
This is when blades hit and **stay together**.
* **The Physics:** Like two magnets snapping together. Instead of bouncing off, the fighters maintain pressure to protect themselves. If you take your sword away, you might get hit, so you leave it there.
* **The Tactical Result:** "Grappling with swords." You are now connected to the enemy. You can sense if they are pushing hard (Strong) or soft (Weak).
* **Game Logic:**
    * **Event:** `Attack` meets `Defense`.
    * **Result:** Attack damage = 0, but **Status Effect Added: [BIND]**.
    * **Next State:** The swords remain locked at `[2,2]`.
    * **Unlock:** New abilities become available (Winding, Slicing) that are *only* possible because the blades are touching.

### How to visualize it on your 3x3 Grid

**Scenario A: The Deflection (Blade Contact)**
1.  **Attacker** swings from High-Right `[1,3]` to Center `[2,2]`.
2.  **Defender** puts sword in `[2,2]`.
3.  **Collision!**
4.  **Result:** *Clang!* The Attacker is knocked back to `[1,3]`. The Defender stays in `[2,2]`.
5.  **Turn End.**

**Scenario B: The Bind (True Bind)**
1.  **Attacker** swings from High-Right `[1,3]` to Center `[2,2]`.
2.  **Defender** puts sword in `[2,2]`.
3.  **Collision!**
4.  **Result:** *Schling!* The swords slide together.
5.  **Status:** Both players are now "Locked" at `[2,2]`.
6.  **Next Turn:** Because the blades are touching, the Attacker can now choose **Slice (Schnitten)** or **Thrust (Stechen)** with 0 travel cost.

### Summary for your Design
* **Blade Contact** is the **Collision Event** (Did they hit?).
* **The Bind** is the **Post-Collision State** (Did they stick?).

If your game wants to be realistic, "Slicing" (one of your 3 Wounds) should **only** be selectable if the player is currently in a "Bind State." You can't slice someone if you just bounced off their sword.

---

### Bind: Quantifying Fühlen (Strong and Weak Leverage) with The Leverage Track: 
(There is also concept of Hard and Soft, this is about feeling how opponent pushes his force)
...
- `LeverageTrack`: **Stark/Swach Meter**
   - ***Mechanic: **Leverage Track**, scaled from +3 (Max Stark) to -3 (Max Swach)***
     - **Stark State** (+1 to +3): Indicates the Actor possesses structural advantage and safety. Available actions are "Hard" (applying direct pressure, executing a powerful strike, or maintaining the bind).   
     - **Swach State** (-1 to -3): Indicates structural vulnerability. The Actor must utilize "Soft" actions (disengaging, retreating, or Winding) to escape the pressure. Failure to act softly when in a Swach state leads to catastrophic displacement and injury.

  ***The position on Leverage Track is the direct outcome of the players' simultaneous action choices (the Indes resolution). By visualizing this state, the game simulates the tactical judgment required of a master Actor to "feel well if he is weak or strong at the sword".***
...
- **Outcome Modification:** The defender's current Leverage position determines the severity of the incoming wound.
  - A Actor in a high Stark state, even if struck, has adequate structure and defense to mitigate the damage.
  - A Actor in a high Swach state is structurally displaced, making recovery nearly impossible, and guaranteeing a severe outcome.
...
Winden (Winding) is used in the bind. It is the defining technical maneuver of KdF combat from the bind, allowing the Actor to shift their strong section onto the opponent's weak section, often defeating a stronger opponent through technique rather than brute force.
