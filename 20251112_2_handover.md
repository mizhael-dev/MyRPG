...
- ✅ **Timeline visualization implemented** - Film strip behavior with 5 synchronized timebars
- ✅ **Action history tracking** - GameEngine records all completed actions with phase timestamps
- ✅ **View mode decluttering** - PC/NPC views show only relevant timebars (2 instead of 5)
- ✅ **Recovery complete pause** - Auto-pause when fighter finishes recovery phase
- ✅ **Telegraph pause system fixed** - Opponent can act when telegraph reveals
- ✅ **Pause permission checks** - Fighters can't act when it's not their turn
- ✅ **Auto-unpause** - Game resumes after valid action during pause
- ✅ **Combat log filtering** - Messages visible in correct view modes
...
│   ├── commands/
│   │   ├── handover.md                 # Handover command
│   │   └── onboard.md                  # Onboarding command
...
│   │   ├── assets/
│   │   └── CombatSkills/               # Copied from public/
│   │
│   ├── public/
│   │   ├── assets/
│   │   │   └── telegraphs/
...
│   │       └── combos/
│   │           └── slash_slash_thrust.json     # Combo example (not implemented)
│   │
│   ├── scripts/
│   │   └── buildSkills.cjs             # **CSV→JSON build system** (~580 lines)
│   │
│   ├── src/
│   │   ├── components/
│   │   │   ├── Actions/
│   │   │   │   ├── ActionPanel.tsx     # **Action buttons** (skill buttons with tooltips)
│   │   │   │   ├── LineSelectionModal.tsx      # Parry line selection (React Portal)
│   │   │   │   ├── AttackPredictionDropdown.tsx # Deflection attack prediction (React Portal)
│   │   │   │   └── FeintSelectionModal.tsx     # Feint attack selection (React Portal)
│   │   │   ├── Timeline/
│   │   │   │   └── TimelinePanel.tsx   # **5 timebars with film strip behavior**
│   │   │   ├── CombatScene.tsx         # **Main container** (3-column layout, combat log)
│   │   │   └── ViewModeSelector.tsx    # View mode toggle (PC/NPC/Debug)
│   │   ├── engine/
│   │   │   └── GameEngine.ts           # **CORE** - Combat logic, pause system (~890 lines)
│   │   ├── types/
...
│   ├── how to change buttons.md        # User guide for button modifications
│   └── SETUP_GUIDE.md
│
├── Backlog-Later/                      # Future features
│   ├── Player_Character_Sheet.md
...
├── 20251111_1_handover.md              # Button redesign handover
├── 20251112_1_handover.md              # Morning session handover
├── 20251112_Timelines-implementation.md # **Timeline implementation guide**
├── 20251112_2_handover.md              # **THIS FILE**
├── Timelines-design.md                 # **Timeline design specification**
...
13. **`combat-prototype/scripts/buildSkills.cjs`** - CSV→JSON build system
...
```typescript
// 1. Telegraph reveals → Pause triggered
...
- Emergency Defense: 2 flat → 1 damage taken (3 - 2 = 1)
- Retreat: 100% reduction → 0 damage (full dodge)
- Parry: 3 flat → 0 damage (3 - 3 = 0)
- Deflection: 3 flat → 0 damage (3 - 3 = 0)
...
1. `overhead_strike` - Vertical overhead, high line, 
2. `side_slash` - Horizontal slash, horizontal line, 
3. `thrust` - Forward stab, center line, 
4. `upward_strike` - Rising attack, low line, 
5. `diagonal_slash` - Angled cut, diagonal line, 

**Defenses (4):**

1. `emergency_defense` - Last resort, 2 flat reduction
2. `retreat` - Disengage by moving away, 100% reduction (full dodge)
3. `parry` - Line-based block, 3 flat reduction, requires correct line,
4. `deflection` - Attack prediction block, requires attack ID,

**Special (1):**

1. `feint` - Attack morph, reads defender telegraph, small time penalty to new attack
...
- ✅ Film strip timeline with 5 synchronized timebars
- ✅ Phase-colored action bars (green/yellow/grey/red)
- ✅ Action history tracking with pre-calculated phase timestamps
- ✅ View mode filtering (PC/NPC/Debug perspectives)
- ✅ Dynamic layout based on visible timebars
- ✅ Configuration constants with plain-English comments
- ✅ Recovery complete pause with log message
- ✅ Telegraph pause with opponent identification
...
**Average attack speed:** (500ms windUp, 500 committed, total 1000ms to impact, 500 recovery) 
**Fast attack:** (300ms windUp, 400 committed, total 700ms to impact, 500 recovery) 
**Slow attack:** (700ms windUp, 600 committed, total 1300ms to impact, 500 recovery) 
...
**Pause flow:**
- Telegraph reveals → checkAutoPause() → isPaused = true
- User selects skill → executeSkill() checks permission
- If valid → create action + auto-unpause
